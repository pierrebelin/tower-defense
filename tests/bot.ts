...
import { upgradeCost } from '../src/sim/commands';
import type { World } from '../src/sim/World';
import type { Tower } from '../src/sim/types';
  ...
  act(): void {
    ...
    for (let guard = 0; guard < 20; guard++) {
      ...
      if (attackers < wantAttackers && walls.length) {
        ...
        w.dispatch({ c: 'upgrade', tower: best.id, def: fam });
        this.towerCount++;
        continue;
      }
      if (this.planIdx < this.plan.length) {
        ...
        const r = w.dispatch({ c: 'build', def: 'wall', x, y });
      ...
      w.dispatch({ c: 'upgrade', tower: up.id, def: next });
    }
  }
}
