...
import { CAMPAIGN_LENGTH, DIFFICULTY } from '../data/creeps';
...
import type { Command, Creep, Difficulty, GameEvent, MapDef, Phase, Projectile, Result, Tower } from './types';
...
export class World {
  ...
  dispatch(cmd: Command): Result {
    const r = executeCommand(this, cmd);
    if (r.ok) this.log.push({ tick: this.tick, cmd });
    return r;
  }
