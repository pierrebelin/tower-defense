...
import { CAMPAIGN_LENGTH, CREEPS, DIFFICULTY, waveAt } from '../data/creeps';
import { MAP_CROSSING } from '../data/map';
import { BUILD_MENU, TOWERS } from '../data/towers';
import { Effects } from '../render/Effects';
import { Renderer, type ViewState } from '../render/Renderer';
import { PAL } from '../render/palette';
import { drawCreep, drawTower } from '../render/sprites';
import { ARMOR_LABEL, ATTACK_LABEL, ATTACK_TABLE } from '../sim/Damage';
import { canBuild, previewRoute, refundValue, upgradeCost } from '../sim/commands';
import { canLaunchNext } from '../sim/systems/waves';
import { World } from '../sim/World';
import type { ArmorType, AttackType, Creep, Difficulty, GameEvent, TargetMode, Tower } from '../sim/types';
...
export class Game {
  ...
  private build(): void {
    ...
    this.sfx.unlock();
    const r = this.world.dispatch({ c: 'build', def: this.buildDef, x: a.x, y: a.y });
  ...
  private upgrade(t: Tower, to: string): void {
    const r = this.world.dispatch({ c: 'upgrade', tower: t.id, def: to });
    if (!r.ok) this.fail(r.reason);
    else this.drainNow();
    this.hoverSlot = null;
  }
  ...
  private sell(t: Tower): void {
    const r = this.world.dispatch({ c: 'sell', tower: t.id });
    if (!r.ok) this.fail(r.reason);
    else {
      this.drainNow();
      this.selected = null;
  ...
  private callWave(): void {
    ...
    const r = this.world.dispatch({ c: 'callWave' });
    if (!r.ok) this.fail(r.reason);
    else this.drainNow();
  }
      ...
      if (t.def.attack) {
        slots[8] = {
          ...
          run: () => {
            ...
            w.dispatch({ c: 'target', tower: t.id, mode: next });
